...
app.post('/api/seed/july2026', async (req, res) => {
  ...
  try {
    ...
    for (let day = 1; day <= 31; day++) {
      let employee;
      if (day === 1) { employee = bob; }
      else { const block = Math.floor((day - 2) / 2); employee = block % 2 === 0 ? georgiana : bob; }
      ...
      if (employee._id.equals(georgiana._id)) scheduleG.push(label); else scheduleB.push(label);
    }
    ...
    const makeTable = days => `<table style="width:100%;border-collapse:collapse;font-size:14px;"><thead><tr style="background:#3498db;color:#fff;"><th style="padding:8px 12px;text-align:left;">Data</th><th style="padding:8px 12px;text-align:left;">Check In</th><th style="padding:8px 12px;text-align:left;">Check Out</th><th style="padding:8px 12px;text-align:left;">Status</th></tr></thead><tbody>${makeRows(days)}</tbody></table>`;
